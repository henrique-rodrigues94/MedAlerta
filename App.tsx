...
function AppRoot() {
 ...
 useEffect(()=>{ iniciarBanco(); configurarCanalDeAlarme(); configurarNotificacoesPush(); iniciarAds(); obterExpoPushToken(); AsyncStorage.getItem(CHAVE_ONBOARDING).then(v=>setOnboardingConcluido(v==='true'));
  const unsubscribeForeground=registrarListenerDeAlarme((medicamentoId,horarioPrevisto,notificationId)=>abrirAlarme(medicamentoId,horarioPrevisto,notificationId)); registrarBackgroundHandler((medicamentoId,horarioPrevisto,notificationId)=>abrirAlarme(medicamentoId,horarioPrevisto,notificationId)); const unsubscribePush=registrarListenersPush((data:SyncPayload)=>handlePushRecebido(data)); const checkDeepLink=async()=>{const url=await Linking.getInitialURL();if(url)handleDeepLink(url)};checkDeepLink();const deep=Linking.addEventListener('url',({url})=>handleDeepLink(url));return()=>{unsubscribeForeground();unsubscribePush();deep.remove()};},[]);
 function abrirAlarme(medicamentoId:string,horarioPrevisto:string,notificationId:string){const {buscarMedicamentoPorId}=require('./src/db/database');const medicamento=buscarMedicamentoPorId(medicamentoId);if(medicamento)setAlarmeAtivo({medicamento,notificationId,horarioPrevisto});}
 async function handlePushRecebido(data:SyncPayload){if(!data?.tipo)return;if(data.tipo==='vinculacao'&&data.cuidadorNome&&data.cuidadorToken){await salvarCuidador({id:data.cuidadorId||`cuid-${Date.now()}`,nome:data.cuidadorNome,expoPushToken:data.cuidadorToken,vinculadoEm:new Date().toISOString()});Alert.alert('Cuidador vinculado!',`${data.cuidadorNome} agora acompanha seus remédios.`)} if(data.tipo==='status'||data.tipo==='alerta'){const pacientes=await listarPacientes();const paciente=pacientes.find(p=>p.expoPushToken===data.pacienteId);if(paciente)await atualizarStatusPaciente(paciente.id,data.status||'pendente',data.medicamentoNome)}}
 ...
 if(isLoading||onboardingConcluido===null)return <View style={{flex:1,backgroundColor:'#F4F7FB'}}/>; if(!onboardingConcluido)return <><StatusBar style="dark"/><OnboardingScreen onConcluir={concluirOnboarding}/></>; if(!mode)return <><StatusBar style="dark"/><ModeSelectScreen/></>;
